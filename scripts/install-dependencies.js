...
// Adapted from https://github.com/withspectrum/spectrum
...
const rootDirs = getDirs(join(__dirname, '..')).filter((dir) => dir.indexOf('build') === -1)
...
const installDeps = (dir, callback) => {
	const stream = spawn(process.platform === 'win32' ? 'yarn.cmd' : 'yarn', ['install', '--no-progress', '--non-interactive'], {
