...
const addPinToRoute = async (pin: PinType, addToRoute: AddToRouteArgs, ownerUid: string): Promise<RouteType> => {
	...
	const { routeUid, afterPin } = addToRoute
	/* If `afterPin` was supplied, add it to the array of pins in the route. Only used when createing a route */
	const pins = [afterPin, pin.uid].filter(Boolean)
	const newRoute = routeUid
		? await Route.addPin({ routeUid, pinUid: pin.uid, afterPin }, ownerUid)
