...
const isNullOrUndefined = (val: any) =>
	Boolean(val === null || val === undefined)
...
const genericNumberOperator = (operator: string) => (
	field: string,
	value: number | Date,
): string => `${operator}(${field}, ${value.toString()})`
...
const filterStringCreators = {
	...
	notEq: (field: string, value: string | number | boolean | Date): string =>
		`NOT ${filterStringCreators.eq(field, value)}`,
	...
	between: (field: string, { start, end }: Range) => {
		if (isNullOrUndefined(start))
			throw new Error(`Range filter: "start" must not be empty`)
		if (isNullOrUndefined(end))
			throw new Error(`Range filter: "end" must not be empty`)
		return `(${filterStringCreators.gte(
			field,
			start,
		)} AND ${filterStringCreators.lte(field, start)})`
	},
	before: genericNumberOperator('le'),
	after: genericNumberOperator('ge'),

	includes: (field: string, value: string) =>
		`allofterms(${field}, "${value}")`,
}
...
const createOperatorFilter = (
	...
	if (!filterStringCreators[operator])
