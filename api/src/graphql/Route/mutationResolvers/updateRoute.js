...
export const updateRoute = async (
	...
	if (!ctx.viewer) throw Error('You must be logged in to create new routes. Please log in and try again.')
	const existingRoute = await ctx.models.Route.getRoute(input.uid)
	if (!existingRoute) throw new Error('This route does not exist')
	if (existingRoute.owner.uid !== ctx.viewer.uid) throw new Error('You can only update your own routes')
