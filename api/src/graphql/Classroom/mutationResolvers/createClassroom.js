...
export const createClassroom = async (
	...
	const { viewer } = ctx
	if (!viewer) throw new UserError('You must be logged in to create new maps. Please log in and try again.')
	if (!viewer.roles.includes('teacher') && !viewer.roles.includes('admin')) {
		throw new UserError('You must be a teacher or admin to create a new classroom.')
	}
	if (assignTeachers && !viewer.roles.includes('admin'))
		throw new UserError('You must be an admin to create classrooms for other teachers')
	...
	if (viewer.roles.includes('admin') && assignTeachers) {
		await Promise.all(assignTeachers.map((t) => () => ctx.models.Classroom.assignTeacher(newClassroom.uid, t)))
	}
	return newClassroom
}
