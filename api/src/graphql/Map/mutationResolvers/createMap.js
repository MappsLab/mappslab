...
export const createMap = async (
	...
	if (!addToClassrooms || addToClassrooms.length === 0) {
		throw new Error(
			'You must supply at least one classroom UID in `addToClassrooms`',
		)
	}
	await Promise.all(
		addToClassrooms.map(async (classroomUid) => {
			...
			if (!classroom)
				throw new UserError(
					`A classroom with the uid "${classroomUid}" does not exist`,
				)
			/* Make sure the viewer teaches in the classroom or is admin */
			if (!viewer.roles.includes('admin')) {
				const viewerTeachesInClassroom = await ctx.models.User.userTeachesInClassroom(
					viewer.uid,
					classroomUid,
				)
				if (viewerTeachesInClassroom === false) {
					throw new UserError(
						'You must be an admin to add a map to a classroom that you do not teach in',
					)
				}
			}
		}),
