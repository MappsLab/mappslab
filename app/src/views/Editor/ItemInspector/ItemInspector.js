			...
			{({ data }) => (
				<CustomPopup position={position}>
					<Pane size="small">
						...
						{item.__typename === 'Pin' ? (
							<PinInspector mapUid={mapUid} pin={item} viewer={data && data.currentViewer && data.currentViewer.viewer} />
						) : item.__typename === 'Route' ? (
							<RouteInspector route={item} mapUid={mapUid} viewer={data && data.currentViewer && data.currentViewer.viewer} />
						) : null}
					</Pane>
				</CustomPopup>
			)}
